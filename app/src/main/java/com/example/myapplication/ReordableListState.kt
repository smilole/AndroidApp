...
class ReorderableListState(
    val lazyListState: LazyListState, private val onMove: (Int, Int) -> Unit
) {
    ...
    fun onDragStart(offset: Offset) {
        lazyListState.layoutInfo.visibleItemsInfo.firstOrNull { item -> offset.y.toInt() in item.offset..(item.offset + item.size) }
            ?.also {
    ...
    fun onDrag(offset: Offset) {
        ...
        initialOffsets?.let { (topOffset, bottomOffset) ->
            ...
            currentElement?.let { hovered ->
                lazyListState.layoutInfo.visibleItemsInfo.filterNot { item -> item.offsetEnd < startOffset || item.offset > endOffset || hovered.index == item.index }
                    .firstOrNull { item ->
